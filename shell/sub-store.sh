...
#
# 基于 v6.6 版本修改：
# 1. [回退] 根据用户反馈，将 Nginx 反代流程恢复为“显示指南 + 可选自动化”模式。
# 2. [回退] Nginx 自动化流程恢复为使用 Certbot 申请证书，不再依赖预置证书。
# ==============================================================================
...
do_install() {
    ...
    log_info "FNM 安装完成。"
    log_info "正在使用 FNM 安装 Node.js 和 PNPM..."; fnm install v20.18.0; fnm use v20.18.0
...
manage_menu() {
    while true; do
        ...
        echo -e "${WHITE}--- Sub-Store 管理菜单 (v6.7) ---${NC}\n"
...
main_menu() {
    while true; do
        clear; echo -e "${WHITE}=====================================${NC}"; echo -e "${WHITE}     Sub-Store 管理脚本 (v6.7)       ${NC}"; echo -e "${WHITE}=====================================${NC}\n"
